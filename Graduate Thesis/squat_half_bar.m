...
% xlabel("膝の位置");
% ylabel("膝関節トルク[N・m]");
% title("膝の位置ごとの膝関節発揮トルク(ハーフ)","バーの重さ=体重");
% text(0.92,390,{append('n=', num2str(sz3(1)))});
% text(1.93,359,{append('n=', num2str(sz4(1)))});
...
xlabel("膝の足関節からの位置[m]");
ylabel("膝関節トルク[N・m]");
title("膝の位置ごとの膝関節発揮トルク(ハーフ)","バーの重さ = 体重×2");
...
plot(pict_lb(7),pict_lb(8)+0.18,'ok','MarkerSize',50);
...
plot(pict_rb(7),pict_rb(8)+0.16,'ok','MarkerSize',50);
plot([len_toe len_toe],[0 1.8],'-b')
ylim([0 1.8])
xlim([-0.4 0.7])

%% スティックピクチャ右上
pict_rt = [pknee2(50,:) phip2(50,:) phead2(50,:) pbar2(50,:)];
figure(11)
hold on
plot([len_toe -len_heel],[0 0],'-k','LineWidth',10);
...
plot(pict_rt(7)-0.074,pict_rt(8),'ok','MarkerSize',50);
plot([len_toe len_toe],[0 1.8],'-b')
ylim([0 1.8])
xlim([-0.4 0.7])
