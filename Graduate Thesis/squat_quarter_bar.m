...
% xlabel("膝の位置");
% ylabel("膝関節トルク[N・m]");
% title("膝の位置ごとの膝関節発揮トルク(クォーター)","バーの重さ=体重");
% text(0.93,400,{append('n=', num2str(sz3(1)))});
% text(1.91,362,{append('n=', num2str(sz4(1)))});
% ylim([-200 410])
...
xlabel("膝の足関節からの位置[m]");
ylabel("膝関節トルク[N・m]");
title("膝の位置ごとの膝関節発揮トルク(クォーター)","バーの重さ = 体重×2");

%% スティックピクチャ左下
pict_lb = [pknee3(end-56,:) phip3(end-56,:) phead3(end-56,:) pbar3(end-56,:)];
figure(9)
hold on
...
plot(pict_lb(7),pict_lb(8)+0.168,'ok','MarkerSize',50);
plot([len_toe len_toe],[0 2],'-b')
ylim([0 2])
xlim([-0.5 0.7])
...
plot(pict_rb(7)-0.075,pict_rb(8)+0.075,'ok','MarkerSize',50);
plot([len_toe len_toe],[0 2],'-b')
ylim([0 2])
xlim([-0.5 0.7])

%% スティックピクチャ右上
pict_rt = [pknee2(45,:) phip2(45,:) phead2(45,:) pbar2(45,:)];
figure(11)
hold on
plot([len_toe -len_heel],[0 0],'-k','LineWidth',10);
...
plot(pict_rt(7)-0.075,pict_rt(8)-0.075,'ok','MarkerSize',50);
plot([len_toe len_toe],[0 2],'-b')
ylim([0 2])
xlim([-0.5 0.7])